  ...
  return (
    <div style={{ padding: 'var(--space-3) var(--space-4) 120px', background: '#f8fffe', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', right: '120px', top: '-40px', width: '200px', height: '200px', borderRadius: '50%', background: 'rgba(255,255,255,0.05)', pointerEvents: 'none' }} />
        <div style={{ zIndex: 1 }}>
          <div style={{ display: 'inline-block', background: 'rgba(255,255,255,0.15)', borderRadius: '999px', padding: '4px 14px', fontSize: '0.78rem', fontWeight: 600, color: 'rgba(255,255,255,0.9)', marginBottom: '12px' }}>
            🗺️ Explore LearnHub
          </div>
          <h2 style={{ color: 'white', margin: '0 0 10px 0', fontFamily: 'var(--font-display)', fontSize: '1.8rem', lineHeight: 1.2, borderBottom: 'none', padding: 0 }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '10px', padding: '8px 14px', color: 'rgba(255,255,255,0.7)', fontSize: '0.82rem' }}>
            💡 Personalized recommendations coming soon!
          </div>
        </div>
        <div style={{ fontSize: '6rem', animation: 'characterBounce 2.5s ease-in-out infinite', zIndex: 1 }}>🦉</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 'var(--space-3)' }}>
        ...
        <div>
          ...
          {available.length === 0 ? (
            <div style={{ background: 'white', borderRadius: '24px', padding: 'var(--space-4)', textAlign: 'center', border: '2px dashed #d1fae5', marginBottom: 'var(--space-3)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 'var(--space-2)', marginBottom: 'var(--space-3)' }}>
              {available.map((course, index) => {
                ...
                return (
                  ...
                  >
                    <div style={{ background: details.color, padding: 'var(--space-2) var(--space-3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div style={{ width: '52px', height: '52px', borderRadius: '14px', background: 'rgba(255,255,255,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>
                        {details.icon}
                      </div>
                      <span style={{ background: 'rgba(255,255,255,0.8)', color: 'var(--ink)', borderRadius: '999px', padding: '3px 10px', fontSize: '0.72rem', fontWeight: 700 }}>
                        {details.tag}
                      </span>
                    </div>
                    <div style={{ padding: 'var(--space-2) var(--space-3) var(--space-3)' }}>
                      <h3 style={{ margin: '0 0 6px 0', fontSize: '1rem', color: 'var(--ink)' }}>{course.title}</h3>
                      <p style={{ fontSize: '0.85rem', color: '#6b7280', lineHeight: 1.5, margin: '0 0 var(--space-2) 0' }}>{details.desc}</p>
                      <button
                        ...
                        style={{
                          ...
                          margin: 0, transition: 'all 0.2s', opacity: isEnrolling ? 0.7 : 1
                        }}
                      >
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-1)' }}>
                {enrolled.filter(c => c.progress < 100).map((course, index) => (
                  <div key={course.id} onClick={() => onNavigateToCourse(course.id)} style={{
                    background: 'white', borderRadius: '16px', padding: 'var(--space-2) var(--space-3)',
                  ...
                  >
                    <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: ['#e8f5e9', '#fef3c7', '#dbeafe'][index % 3], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', flexShrink: 0 }}>
                      {['🧠', '💡', '🔢'][index % 3]}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--ink)', marginBottom: '4px' }}>{course.title}</div>
                    ...
                    <button style={{ background: 'var(--green)', color: 'white', border: 'none', padding: '8px 14px', borderRadius: '10px', fontFamily: 'var(--font-body)', fontSize: '0.82rem', fontWeight: 600, textTransform: 'none', letterSpacing: 'normal', cursor: 'pointer', margin: 0, whiteSpace: 'nowrap' }}>
                      Continue →
                    </button>
                  </div>
                ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
          ...
          <div style={{ background: 'white', borderRadius: '20px', padding: 'var(--space-3)', border: '1px solid #f3f4f6', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
            <h3 style={{ margin: '0 0 var(--space-2) 0', fontSize: '0.95rem', color: 'var(--ink)' }}>🏷️ Browse by Topic</h3>
            <p style={{ color: '#9ca3af', fontSize: '0.78rem', margin: '0 0 var(--space-1) 0' }}>Topic filtering coming soon!</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {topics.map(topic => (
                <div key={topic.label} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 12px', borderRadius: '10px', background: topic.color, opacity: 0.75, cursor: 'not-allowed' }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #fef3c7, #fde68a)', borderRadius: '20px', padding: 'var(--space-3)', border: '1px solid #fcd34d' }}>
            <h3 style={{ margin: '0 0 6px 0', fontSize: '0.95rem', color: '#92400e' }}>🎯 What do you want to be?</h3>
            <p style={{ color: '#b45309', fontSize: '0.78rem', margin: '0 0 var(--space-2) 0', lineHeight: 1.4 }}>Career-based recommendations coming soon!</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
              {careers.map(career => (
                <div key={career.label} style={{ background: 'rgba(255,255,255,0.5)', borderRadius: '10px', padding: '8px', textAlign: 'center', cursor: 'not-allowed', opacity: 0.8 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg, #ede9fe, #ddd6fe)', borderRadius: '20px', padding: 'var(--space-3)', border: '1px solid #c4b5fd', textAlign: 'center' }}>
            <div style={{ fontSize: '3rem', marginBottom: '10px' }}>✨</div>
            <h3 style={{ color: '#4c1d95', margin: '0 0 8px 0', fontSize: '0.95rem' }}>Find the course for you</h3>
            ...
            <button style={{ background: '#7c3aed', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '10px', fontFamily: 'var(--font-body)', fontSize: '0.85rem', fontWeight: 600, textTransform: 'none', letterSpacing: 'normal', cursor: 'not-allowed', margin: 0, opacity: 0.7, width: '100%' }}>
              🦉 Coming Soon!
            </button>
          </div>
        </div>
